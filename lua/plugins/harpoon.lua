return {
  ...
  config = function()
    ...
    vim.keymap.set("n", "<leader>h3", function() harpoon:list():select(3) end, { desc = "Open harpoon window" })
    vim.keymap.set("n", "<leader>h4", function() harpoon:list():select(4) end, { desc = "Open harpoon window" })
    -- Toggle previous & next buffers stored within Harpoon list
    vim.keymap.set("n", "<leader>hp", function() harpoon:list():prev() end, { desc = "Open harpoon window" })
    vim.keymap.set("n", "<leader>hn", function() harpoon:list():next() end, { desc = "Open harpoon window" })
  end
